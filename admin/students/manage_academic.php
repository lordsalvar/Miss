...
<div class="container-fluid">
    <form action="" id="academic-form">
        ...
        <div class="row">
            <div class="col-md-6 form-group">
                ...
                <select name="semester" id="semester" class="form-control form-control-sm  form-control-border rounded-0" required>
                    ...
                    <option <?= (isset($semester) && $semester == 'Third Semester') ? 'selected' : '' ?>>Summer</option>
                </select>
            </div>
        ...
        <div class="row">
            <div class="col-md-6 form-group">
                <label for="course_id" class="control-label">Program</label>
                <select name="course_id" id="course_id" class="form-control form-control-sm form-control-border rounded-0 select2" required>
                    ...
                    <?php 
                        $courses = $conn->query("SELECT c.*, d.name as department FROM `program_list` c inner join `department_list` d on c.department_id = d.id where c.delete_flag = 0 and c.status = 1 ".(isset($course_id) ? " or c.id = '{$course_id}' " : "")." order by d.name asc, c.name asc ");
