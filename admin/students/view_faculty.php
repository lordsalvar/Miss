<?php
if(isset($_GET['id'])){
    $qry = $conn->query("SELECT *, CONCAT(lastname,', ', firstname,' ', middlename) as fullname FROM `faculty_list` where id = '{$_GET['id']}'");
    if($qry->num_rows > 0){
        $res = $qry->fetch_array();
        foreach($res as $k => $v){
            if(!is_numeric($k))
    ...
    $status = isset($status) ? $status : 'N/A';
}
?>
    ...
    <div class="card card-outline card-navy shadow rounded-0">
        <div class="card-header">
            ...
            <div class="card-tools">
                <a class="btn btn-sm btn-primary btn-flat" href="./?page=students/manage_faculty&id=<?= isset($id) ? $id : '' ?>"><i class="fa fa-edit"></i> Edit</a>
                <button class="btn btn-sm btn-danger btn-flat" id="delete_faculty"><i class="fa fa-trash"></i> Delete</button>
                <button class="btn btn-sm btn-navy bg-navy btn-flat" type="button" id="add_academic"><i class="fa fa-plus"></i> Add Academic</button>
                <button class="btn btn-sm btn-info bg-info btn-flat" type="button" id="update_status"><i class="fa fa-sync"></i> Update Status</button>
                <button class="btn btn-sm btn-success bg-success btn-flat" type="button" id="print"><i class="fa fa-print"></i> Print</button>
                <a href="./?page=students/findex" class="btn btn-default border btn-sm btn-flat"><i class="fa fa-angle-left"></i> Back to List</a>
            </div>
        </div>
        <div class="card-body">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            <label class="control-label text-muted">Faculty Roll</label>
                ...
                <fieldset>
                    ...
                    <table class="table table-stripped table-bordered" id="academic-history">
                        ...
                        <tbody>
                            <?php 
                            $i= 1;
                            $academics = $conn->query("SELECT a.*,c.name as course, d.name as department FROM `academic_history` a inner join course_list c on a.course_id = c.id inner join department_list d on c.department_id = d.id where faculty_id = '{$id}' order by a.school_year asc, d.name asc, c.name asc ");
                                ...
                                <?php 
                                    switch($row['end_status']){
                                        ...
                                        case '1':
                                            ...
                                            break;
                                        case '2':
                                            echo '<span class="rounded-pill badge badge-default border bg-maroon px-3">Drop out</span>';
                                            break;
                                        case '3':
...
<noscript id="print-header">
    <div class="row">
        ...
        <div class="col-8">
            ...
            <h3 class="text-center"><b>Faculty Records</b></h3>
        </div>
        <div class="col-2"></div>
    </div>
</noscript>
<script>
    $(function() {
        $('#update_status').click(function(){
            uni_modal("Update Status of <b><?= isset($roll) ? $roll : "" ?></b>","faculty/update_status.php?faculty_id=<?= isset($id) ? $id : "" ?>")
        })
        $('#add_academic').click(function(){
            uni_modal("Add Academic Record for <b><?= isset($roll) ? $roll.' - '.$fullname : "" ?></b>","faculty/manage_academic.php?faculty_id=<?= isset($id) ? $id : "" ?>",'mid-large')
        })
        $('.edit_academic').click(function(){
            uni_modal("Edit Academic Record of <b><?= isset($roll) ? $roll.' - '.$fullname : "" ?></b>","faculty/manage_academic.php?faculty_id=<?= isset($id) ? $id : "" ?>&id="+$(this).attr('data-id'),'mid-large')
        })
        $('.delete_academic').click(function(){
			_conf("Are you sure to delete this Faculty's Academic Record?","delete_academic",[$(this).attr('data-id')])
		})
        $('#delete_faculty').click(function(){
			_conf("Are you sure to delete this Faculty Information?","delete_faculty",['<?= isset($id) ? $id : '' ?>'])
		})
        $('.view_data').click(function(){
			uni_modal("Report Details","faculty/view_report.php?id="+$(this).attr('data-id'),"mid-large")
		})
        ...
        $('#print').click(function(){
            ...
            _el.append(_h)
            _el.append(_ph)
            _el.find('title').text('Faculty Records - Print View')
            _el.append(_p)
    ...
    function delete_student($id){
		start_loader();
		$.ajax({
			url:_base_url_+"classes/Master.php?f=faculty_delete",
			method:"POST",
			data:{id: $id},
			dataType:"json",
			error:err=>{
