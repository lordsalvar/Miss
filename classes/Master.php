...
class Master extends DBConnection
{
	...
	public function save_program_outcome()
	{
		...
		try {
			// Get course details using catalog_number
			$course_qry = $this->conn->query("SELECT id, program_id FROM course_list WHERE catalog_number = '{$catalog_number}'");
			if ($course_qry->num_rows == 0) {
				throw new Exception('Course not found');
			}

			$course_data = $course_qry->fetch_assoc();
			$course_id = $course_data['id'];

			// Use provided program_id or fallback to course's program_id
			if (empty($program_id)) {
				$program_id = $course_data['program_id'];
			}

			if (empty($program_outcomes)) {
				$program_outcomes = '';  // Allow empty program outcomes to clear all
			}

			// Update the course_list table with the selected program outcomes
			$sql = "UPDATE course_list SET program_outcomes = ? WHERE catalog_number = ?";
			$stmt = $this->conn->prepare($sql);
			$stmt->bind_param("ss", $program_outcomes, $catalog_number);
			...
			if ($stmt->execute()) {
				...
				throw new Exception('Failed to save program outcomes');
			}
		} catch (Exception $e) {
			error_log("Error in save_program_outcome: " . $e->getMessage());
			return json_encode([
				'status' => 'failed',
				'msg' => $e->getMessage()
			]);
	...
	public function add_program_outcome()
	{
		...
		try {
			...
			$course_check = $this->conn->query("SELECT id FROM course_list WHERE catalog_number = '{$catalog_number}'");
			if ($course_check->num_rows == 0) {
				throw new Exception('Course not found with catalog number: ' . $catalog_number);
			}

			if (empty($program_outcomes)) {
				throw new Exception('No program outcomes selected');
			}
			...
			if ($stmt->execute()) {
				return json_encode([
					'status' => 'success',
					'msg' => 'Program outcomes successfully saved for ' . $catalog_number
				]);
			}
			throw new Exception('Failed to save program outcomes');
		} catch (Exception $e) {
			error_log("Error in add_program_outcome: " . $e->getMessage());
			return json_encode([
				'status' => 'failed',
				'msg' => $e->getMessage()
			]);
	...
	public function edit_program_outcome()
	{
		extract($_POST);
		...
		try {
			...
			// Get course details and existing program outcomes
			$course_qry = $this->conn->query("SELECT id, program_outcomes FROM course_list WHERE catalog_number = '{$catalog_number}'");
			if ($course_qry->num_rows == 0) {
				throw new Exception('Course not found with catalog number: ' . $catalog_number);
			}

			$course_data = $course_qry->fetch_assoc();
			$existing_outcomes = !empty($course_data['program_outcomes']) ?
				explode(',', $course_data['program_outcomes']) : [];

			// Get array of new outcomes
			$new_outcomes = !empty($program_outcomes) ? explode(',', $program_outcomes) : [];

			// Only add outcomes that don't already exist
			$final_outcomes = array_unique(array_merge($existing_outcomes, $new_outcomes));
			$final_outcomes_str = implode(',', array_filter($final_outcomes));

			// Update with combined outcomes
			$sql = "UPDATE course_list SET program_outcomes = ? WHERE catalog_number = ?";
			$stmt = $this->conn->prepare($sql);
			$stmt->bind_param("ss", $final_outcomes_str, $catalog_number);

			if ($stmt->execute()) {
				return json_encode([
					'status' => 'success',
					'msg' => 'Program outcomes updated successfully for ' . $catalog_number
				]);
			}
			throw new Exception('Failed to update program outcomes');
		} catch (Exception $e) {
			error_log("Error in edit_program_outcome: " . $e->getMessage());
			return json_encode([
				'status' => 'failed',
				'msg' => $e->getMessage()
			]);
	...
	public function get_course_program_outcomes()
	{
		extract($_POST);

		$program_outcomes = $this->conn->query("SELECT * FROM program_outcomes WHERE program_id = '$program_id' ORDER BY po_code ASC");
		$course_outcomes = $this->conn->query("SELECT program_outcomes FROM course_list WHERE id = '$course_id'");

		$all_outcomes = array();
		while ($row = $program_outcomes->fetch_assoc()) {
			$all_outcomes[] = $row;
		}

		$existing_outcomes = array();
		if ($course_row = $course_outcomes->fetch_assoc()) {
			if (!empty($course_row['program_outcomes'])) {
				$existing_outcomes = explode(',', $course_row['program_outcomes']);
			}
		}

		return json_encode(array(
			'status' => 'success',
			'program_outcomes' => $all_outcomes,
			'course_outcomes' => $existing_outcomes
		));
	}
}
...
switch ($action) {
	...
	case 'save_program_outcome':
		...
		break;
	case 'add_program_outcome':
		echo $Master->add_program_outcome();
		break;
	case 'edit_program_outcome':
		echo $Master->edit_program_outcome();
		break;
	...
	case 'get_student_grades':
		...
		break;
	case 'get_course_program_outcomes':
		echo $Master->get_course_program_outcomes();
		break;
	default:
		// echo $sysset->index();
		break;
